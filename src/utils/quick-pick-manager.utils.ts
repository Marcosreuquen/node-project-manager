...
export class QuickPickManager {
     ...
     */
    static async showInstalledDependencies(action: 'remove' | 'update'): Promise<string | undefined> {
    ...
    static async showAvailableScripts(): Promise<string | undefined> {
        ...
        try {
            ...
            const items: QuickPickAction[] = Object.entries(packageJson.scripts).map(([name, command]) => ({
                label: `$(play) ${name}`,
